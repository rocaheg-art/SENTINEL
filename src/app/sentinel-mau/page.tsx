  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-background text-foreground font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        {/* OFFLINE / SIMULATED STATE BANNER */}
        {offline && (
          <div className="bg-card-bg border border-card-border rounded-[28px] p-6 flex flex-col md:flex-row items-center gap-4 text-left max-w-3xl mx-auto shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xs font-bold text-foreground uppercase tracking-wide">Base de datos de Respaldo Local (Modo Vercel/Simulado)</h2>
              <p className="text-[10.5px] text-text-muted mt-0.5 leading-relaxed font-sans">
                No detectamos conexión con el servidor SQL Server local de tu PC (<strong className="text-foreground">SENTINEL_DB</strong>). Para fines de visualización y pruebas de despliegue en Vercel, hemos cargado una copia en memoria de la base de datos de Mauricio.
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={() => setOffline(false)}
                className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-[10px] font-bold cursor-pointer hover:bg-blue-700 shadow-sm"
              >
                Explorar datos locales
              </button>
              <button
                onClick={loadDashboard}
                className="px-3 py-1.5 bg-card-bg border border-card-border text-[10px] font-bold text-text-muted hover:text-foreground rounded-lg cursor-pointer"
              >
                Reintentar BD Real
              </button>
            </div>
